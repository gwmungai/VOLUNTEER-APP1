                    ...
                    if (task.isSuccessful) {
                        ...
                        regRef.setValue(userData).addOnCompleteListener { regTask ->
                            ...
                            if (regTask.isSuccessful) {
                                ...
                                navController.navigate(com.example.volunteerapp.navigation.ROUTE_HOME)
                            } else {
                                showToast(regTask.exception?.message ?: "Registration failed")
                                navController.navigate(com.example.volunteerapp.navigation.ROUTE_LOGIN)
                            }
                        }
                    } else {
                        progress.dismiss()
                        showToast(task.exception?.message ?: "Sign up failed")
                        navController.navigate(com.example.volunteerapp.navigation.ROUTE_SIGNUP)
                    }
                }
            }
        }
